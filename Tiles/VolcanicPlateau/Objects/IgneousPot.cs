...
{
    public class IgneousPot : ModTile
    {
        ...
        public override void KillMultiTile(int i, int j, int frameX, int frameY)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                Dust dust1 = Main.dust[Dust.NewDust(new Vector2(i, j) * 16, 32, 32, 54, 0, -1, 0, new Color(195, 222, 155), 0.5f)];
                Dust dust2 = Main.dust[Dust.NewDust(new Vector2(i, j) * 16, 32, 32, 6, 0, 0, 0, default, 1f)];
                dust2.velocity *= 0.5f;
            }
            int type = frameY / 36;
            for (int g = 0; g < 2; g++)
            {
                Gore.NewGore(new Vector2((int)i * 16 + Main.rand.Next(-10, 10), (int)j * 16 + Main.rand.Next(-10, 10)), new Vector2(-1, 1), mod.GetGoreSlot("Gores/IgneousPot" + (g + type * 2)));
            }
            ...
            else if (Main.rand.NextBool(15) || (Main.rand.NextBool(15) && Main.expertMode) || (Main.rand.NextBool(15) && MyWorld.awakenedMode)) // 1/15 noraml, 2/15 expert, 3/15 awakened
            {
                for (int k = 0; k < 3; k++)
                {
                    Projectile proj = Main.projectile[Projectile.NewProjectile(new Vector2(i + 1, j) * 16, Vector2.Zero, ProjectileID.GreekFire1 + Main.rand.Next(3), Main.hardMode ? 34 : 10, 0, Main.myPlayer)];
                    proj.velocity.Y = Main.rand.NextFloat(-1, 0);
                    proj.velocity.X = Main.rand.NextFloat(-1, 1);
                }
            }
            ...
            else
            {
                ...
                else if (num9 == 5 && (double)j > Main.rockLayer)
                {
                    ...
                    Item.NewItem(i * 16, j * 16, 16, 16, ItemID.Bomb, stack);
                }
                else
                {
                    // money
