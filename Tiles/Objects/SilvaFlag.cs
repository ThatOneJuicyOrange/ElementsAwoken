...
{
    public class SilvaFlag : ModTile
    {
        ...
        {
            Item.NewItem(i * 16, j * 16, 32, 16, ModContent.ItemType<SilvaFlagItem>());
        }
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (t.frameY == 0 || t.frameY == 18)
            {
                ...
                float windScale = MathHelper.Clamp(-Main.windSpeed / 0.6f, -1f, 1f);
                float rot = 100 * (windScale + (float)(Math.Sin((float)MyWorld.generalTimer / 120f) * 0.05f));
