...
namespace ElementsAwoken.BaseMod
{
    public class BaseWorldGenTex
    {
        ...
        public static TexGen GetPlatformTexGenerator(Texture2D platformTex, Dictionary<Color, int> colorToPlatform) // platforms wont generate using above for some reason
        {
            ...
            for (int m = 0; m < platformData.Length; m++)
            {
                ...
                x++;
                if (x >= platformTex.Width) { x = 0; y++; }
                if (y >= platformTex.Height) break; //you've somehow reached the end of the texture! (this shouldn't happen!)
            }
    ...
    public class TexGen
    {
        ...
        public void Generate(int x, int y, bool silent, bool sync, bool removeSlopes = false, bool flip = false)
        {
            if (!flip)
            {
                ...
                {
                    for (int y1 = 0; y1 < height; y1++) // maybe make it generate backwards if furniture needs to work as it will put the tiles below it first and not destroy furniture
                    {
                        int x2 = x + x1, y2 = y + y1;
                        TileInfo info = tileGen[x1, y1];
                        //if (info.tileID == TileID.Books) Main.NewText("B O O K");
                        // EA MODIFCATIONS
