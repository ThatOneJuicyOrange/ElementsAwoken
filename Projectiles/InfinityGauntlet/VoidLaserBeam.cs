...
namespace ElementsAwoken.Projectiles.InfinityGauntlet
{
    ...
    public class VoidLaserBeam : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (voidballTimer <= 0)
            {
                Vector2 perturbedSpeed = new Vector2(projectile.velocity.X, projectile.velocity.Y).RotatedByRandom(MathHelper.ToRadians(10)) * 15;
                Projectile.NewProjectile(projectile.position.X, projectile.position.Y, perturbedSpeed.X, perturbedSpeed.Y, mod.ProjectileType("InfiniyVoidBolt"), projectile.damage, 4f, player.whoAmI);
            ...
            else
            {
                ...
                for (int k = 0; k < chargeFact + 1; k++)
                {
                    ...
                    Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, 127, projectile.velocity.X / 2f,
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, 127, dustVel.X, dustVel.Y, 0, new Color(), 1f)];
                dust.noGravity = true;
                dust.scale = 1.2f;
            ...
            if (Main.rand.Next(5) == 0)
            {
                Vector2 offset = projectile.velocity.RotatedBy(1.57f, new Vector2()) * ((float)Main.rand.NextDouble() - 0.5f) *
                                 projectile.width;
                Dust dust = Main.dust[
                    Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, 127, 0.0f, 0.0f, 100, new Color(), 1.5f)];
                ...
                unit.Normalize();
                dust = Main.dust[
                    Dust.NewDust(Main.player[projectile.owner].Center + 55 * unit, 8, 8, 127, 0.0f, 0.0f, 100, new Color(), 1.5f)];
                dust.velocity = dust.velocity * 0.5f;
                dust.velocity.Y = -Math.Abs(dust.velocity.Y);
            }
            #endregion
