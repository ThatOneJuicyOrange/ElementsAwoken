...
{
    public class RegarothBeam : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Regaroth Beam");
        }
        public override void AI()
        {
			...
			if (projectile.localAI[0] > 2f)
			{
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Dust dust = Main.dust[Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, dustType, 0f, 0f, 100, default(Color), 1.5f)];
                    dust.noGravity = true;
                    dust.velocity *= 0f;
                }
			}
