...
namespace ElementsAwoken.Projectiles.NPCProj.Ancients
{
    public class CrystallineKunaiHostileExplosive : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            int projType = projectile.ai[0] == 0 ? mod.ProjectileType("CrystallineShard") : mod.ProjectileType("CrystallineShardHoming");
            float speed = projectile.ai[0] == 0 ? 4f : 7f;
            for (int i = 0; i < numProj; i++)
            {
                Vector2 perturbedSpeed = new Vector2(1f, 1f).RotatedBy((rotation / numProj) * i) * speed;
                Projectile proj = Main.projectile[Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, projType, projectile.damage, 2f, 0)];
