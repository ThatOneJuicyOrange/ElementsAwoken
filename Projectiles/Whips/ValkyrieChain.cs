...
namespace ElementsAwoken.Projectiles.Whips
{
    public class ValkyrieChain : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Projectile.NewProjectile(target.Center.X, target.Center.Y, Main.rand.Next(-10,10), Main.rand.Next(-10, 10), mod.ProjectileType("ValkyrieBolt"), projectile.damage, 0, projectile.owner);
            }
            Projectile.NewProjectile(target.Center.X, target.Center.Y, 0, 0, mod.ProjectileType("RainbowExplosion"), projectile.damage, 0, projectile.owner);
        }
    }
}