        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] > 10f)
            {
                ...
                for (int k = 0; k < num16 + 1; k++)
                {
                    ...
                    int num20 = Dust.NewDust(vector14 - Vector2.One * 8f, 16, 16, mod.DustType("AncientGreen"), projectile.velocity.X / 2f, projectile.velocity.Y / 2f, 0, default(Color), 1f);
                ...
                if (player.channel && !player.noItems && !player.CCed)
                {
                    if (projectile.ai[0] >= chargeLevel3)
                    {
                        ...
                        for (int i = 0; i < numberProjectiles; i++)
                        {
                            ...
                            Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, mod.ProjectileType("DesolationCharge4"), damage, projectile.knockBack, player.whoAmI);
                        }
                        Main.PlaySound(SoundID.Item113, projectile.position);
                        projectile.Kill();
                    }
                }
                else
                {
                    int projType = mod.ProjectileType("DesolationCharge1");
                    if (projectile.ai[0] < chargeLevel1)
                    {
                        projType = mod.ProjectileType("DesolationCharge1");
                    }
                    else if (projectile.ai[0] < chargeLevel2)
                    {
                        projType = mod.ProjectileType("DesolationCharge2");
                    }
                    else if (projectile.ai[0] < chargeLevel3)
                    {
                        projType = mod.ProjectileType("DesolationCharge3");
                    }
