...
namespace ElementsAwoken.NPCs.ItemSets.Carapace
{
    ...
    {
        public override string Texture { get { return "ElementsAwoken/NPCs/ItemSets/Carapace/Pebleer"; } }
        ...
        {
            DisplayName.SetDefault("Stoneer");
        ...
        {
            Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemType<CarapaceItem>(), Main.rand.Next(3, 6));
            if (Main.rand.NextBool(4)) Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemType<PebleerEgg>(), 1);
        }
                ...
                if (Math.Abs(npc.velocity.X) < 0.05f || timer > 300 || Math.Abs(P.Center.X - npc.Center.X) > 600)
                {
                    if (Math.Abs(npc.velocity.X) < 0.05f)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            {
                               Projectile.NewProjectile(npc.Center.X, npc.Center.Y, Main.rand.NextFloat(2, 6) * -rollDir, Main.rand.NextFloat(-6, -2), ProjectileType<StoneerP>(), npc.damage, 3, Main.myPlayer);
                            }
                        }
                        rollDir = 3;
                    }
