...
{
    public class MoltenFlamewisp : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            npc.GetGlobalNPC<PlateauNPCs>().tomeText = "Molten flamewisps are self-aware chunks of obsidian imbued with ectoplasm thanks to the Void Leviathan burrowing through the dungeon on its way here. They have been seen hunting down and killing Voidbroken creatures.";
        }
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Molten Flamewisp");
        }
        ...
        public override void AI()
        {
            ...
            if (Vector2.Distance(npc.Center, targetPos) < attackDist)
            {
                ...
                if (npc.ai[2] >= 0)
                {
                    ...
                    float speed = 3f;
                    Projectile proj = Main.projectile[Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)((Math.Cos(rotation) * speed) * -1), (float)((Math.Sin(rotation) * speed) * -1), ProjectileType<ForgottenWispBolt>(), npc.damage / 2, 0f, Main.myPlayer, 0f, 0f)];
                    proj.extraUpdates = 4;
                    proj.timeLeft = 90;
                    proj.localAI[0] = 90;
                    npc.ai[2] = -90;
